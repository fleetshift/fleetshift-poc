...
func verifyEnrollmentIDToken(ctx context.Context, config Config, nonce, rawToken string) (oidcIdentity, error) {
	...
	discoveryRequest, err := http.NewRequestWithContext(ctx, http.MethodGet, strings.TrimRight(config.TrustManifest.OIDCIssuer, "/")+"/.well-known/openid-configuration", nil)
	if err != nil {
	...
	if discovery.Issuer != config.TrustManifest.OIDCIssuer || discovery.JWKSURI == "" {
	...
	token, err := jwt.ParseString(rawToken,
		...
		jwt.WithIssuer(config.TrustManifest.OIDCIssuer),
		jwt.WithAudience(config.TrustManifest.EnrollmentClientID),
	)
	if err != nil {
