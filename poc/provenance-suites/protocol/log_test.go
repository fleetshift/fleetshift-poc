...
func TestVerifyLogUpdateStartsFromEmptyTree(t *testing.T) {
	tree := merklelog.New()
	evidence := testLogEvidence("first")
	update := mustAppendLogUpdate(t, tree, EmptyCheckpoint(), evidence)
	...
	if err := VerifyLogUpdate(EmptyCheckpoint(), update, evidence); err != nil {
		t.Fatalf("verify first update: %v", err)
	}
}

func TestVerifyLogUpdateRequiresLeafToEqualRootIdentity(t *testing.T) {
	tree := merklelog.New()
	evidence := testLogEvidence("bound")
	update := mustAppendLogUpdate(t, tree, EmptyCheckpoint(), evidence)
	other := testLogEvidence("other")

	if err := VerifyLogUpdate(EmptyCheckpoint(), update, other); !errors.Is(err, ErrInvalidLogUpdate) {
		t.Fatalf("mismatched evidence error = %v, want ErrInvalidLogUpdate", err)
	}

	update.Leaf = mustIdentity(t, other)
	if err := VerifyLogUpdate(EmptyCheckpoint(), update, evidence); !errors.Is(err, ErrInvalidLogUpdate) {
		t.Fatalf("mismatched leaf error = %v, want ErrInvalidLogUpdate", err)
	}
}

func TestVerifyLogUpdateAcceptsAppendAndRetry(t *testing.T) {
	tree := merklelog.New()
	firstEvidence := testLogEvidence("one")
	first := mustAppendLogUpdate(t, tree, EmptyCheckpoint(), firstEvidence)
	if err := VerifyLogUpdate(EmptyCheckpoint(), first, firstEvidence); err != nil {
		t.Fatalf("verify first append: %v", err)
	}

	secondEvidence := testLogEvidence("two")
	second := mustAppendLogUpdate(t, tree, first.Checkpoint, secondEvidence)
	if second.Index != 1 {
	...
	if err := VerifyLogUpdate(first.Checkpoint, second, secondEvidence); err != nil {
		t.Fatalf("verify second append: %v", err)
	}

	retry := mustLogUpdate(t, tree, second.Checkpoint, second.Index, second.Leaf)
	...
	if err := VerifyLogUpdate(second.Checkpoint, retry, secondEvidence); err != nil {
		t.Fatalf("verify same-head retry: %v", err)
	}
}

func TestVerifyLogUpdateLeafDoesNotIncludeIndex(t *testing.T) {
	...
	first := mustLogUpdate(t, tree, EmptyCheckpoint(), 0, identity)
	second := mustLogUpdate(t, tree, EmptyCheckpoint(), 1, identity)
	...
	if err := VerifyLogUpdate(EmptyCheckpoint(), first, evidence); err != nil {
		t.Fatalf("verify identity at index 0: %v", err)
	}
	if err := VerifyLogUpdate(EmptyCheckpoint(), second, evidence); err != nil {
		t.Fatalf("verify identity at index 1: %v", err)
	}

	otherTree := merklelog.New()
	mustAppendLogUpdate(t, otherTree, EmptyCheckpoint(), evidence)
	mustAppendLogUpdate(t, otherTree, mustCheckpoint(t, otherTree, 1), testLogEvidence("other"))
	moved := mustLogUpdate(t, otherTree, EmptyCheckpoint(), 0, identity)
	moved.Index = 1
	if err := VerifyLogUpdate(EmptyCheckpoint(), moved, evidence); !errors.Is(err, ErrInvalidLogUpdate) {
		t.Fatalf("moved index without a matching inclusion proof error = %v, want ErrInvalidLogUpdate", err)
	}
}

func TestVerifyLogUpdateSkipsUnrelatedLeaf(t *testing.T) {
	tree := merklelog.New()
	unrelated := testLogEvidence("other-target")
	mustAppendLogUpdate(t, tree, EmptyCheckpoint(), unrelated)

	relevant := testLogEvidence("this-target")
	from := mustCheckpoint(t, tree, 1)
	update := mustAppendLogUpdate(t, tree, from, relevant)
	...
	if err := VerifyLogUpdate(EmptyCheckpoint(), mustLogUpdate(t, tree, EmptyCheckpoint(), 1, update.Leaf), relevant); err != nil {
		t.Fatalf("verify inclusion of this leaf under a tree that also contains an unrelated leaf: %v", err)
	}
}

func TestVerifyLogUpdateRejectsTamperingAndForks(t *testing.T) {
	tree := merklelog.New()
	firstEvidence := testLogEvidence("one")
	first := mustAppendLogUpdate(t, tree, EmptyCheckpoint(), firstEvidence)
	secondEvidence := testLogEvidence("two")
	...
	for _, tc := range []struct {
		name   string
		mutate func(*LogUpdate)
	}{
		{name: "leaf", mutate: func(update *LogUpdate) { update.Leaf = mustIdentity(t, testLogEvidence("attacker")) }},
		{name: "inclusion proof", mutate: func(update *LogUpdate) { update.InclusionProof[0] = forkRoot }},
		{name: "consistency proof", mutate: func(update *LogUpdate) { update.ConsistencyProof[0] = forkRoot }},
		{name: "forked root", mutate: func(update *LogUpdate) { update.Checkpoint.Root = forkRoot }},
		{name: "skip-ahead size", mutate: func(update *LogUpdate) { update.Checkpoint.Size = 99 }},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.mutate(&tampered)
			if err := VerifyLogUpdate(first.Checkpoint, tampered, secondEvidence); !errors.Is(err, ErrInvalidLogUpdate) {
...
func mustLogUpdate(t *testing.T, tree *merklelog.Tree, from Checkpoint, index uint64, leaf Digest) LogUpdate {
	t.Helper()
...
func mustAppendLogUpdate(t *testing.T, tree *merklelog.Tree, from Checkpoint, evidence TypedEvidence) LogUpdate {
	t.Helper()
	identity := mustIdentity(t, evidence)
	leafHash, err := LeafHash(identity)
	if err != nil {
	...
	return mustLogUpdate(t, tree, from, index, identity)
}
