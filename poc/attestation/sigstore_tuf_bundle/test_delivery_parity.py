...
from sigstore_tuf_bundle.model import DerivedInput, FulfillmentState
...
class DeliveryVerificationTests(unittest.TestCase):
    ...
    def test_inline_manifest_put_happy_path(self) -> None:
        ...
        self.assertTrue(self.agent.verify(d, target_identity=self.prod_target).valid)
    ...
    def test_inline_manifest_tampered_output(self) -> None:
        ...
        assert_rejected_with(
            self, self.agent, d, "manifests must match inline spec",
    ...
    def test_addon_manifest_wrong_addon_signs(self) -> None:
        ...
        assert_rejected_with(
            self, self.agent, d, "manifests must be signed by observability",
            target_identity=self.prod_target,
        )
    ...
    def test_predicate_placement_target_no_match_put_rejected(self) -> None:
        ...
        d = unsigned_put(self.env, "att-1", si, SAMPLE_MANIFESTS)
        assert_rejected_with(
            self, self.agent, d, "target matches placement predicate for put",
    ...
    def test_predicate_placement_target_no_match_remove_accepted(self) -> None:
        ...
        self.assertTrue(self.agent.verify(self.env.assemble(att), target_identity=self.staging_target).valid)

    def test_predicate_placement_target_matches_remove_rejected(self) -> None:
        si = self._inline_predicate(SAMPLE_MANIFESTS, 'target.labels.env == "prod"')
        att = make_remove_attestation("att-1", si, "deploy-1")
        assert_rejected_with(
            self, self.agent, self.env.assemble(att),
            "removal requires placement predicate non-match",
            target_identity=self.prod_target,
        )
    ...
    def test_addon_placement_signed_evidence_accepted(self) -> None:
        ...
        self.assertTrue(self.agent.verify(self.env.assemble(att), target_identity=self.prod_target).valid)

    def test_addon_placement_missing_evidence_rejected(self) -> None:
        si = self._inline_addon(SAMPLE_MANIFESTS)
        d = unsigned_put(self.env, "att-1", si, SAMPLE_MANIFESTS)
        assert_rejected_with(
            self, self.agent, d, "placement must be signed by capacity-planner",
            target_identity=self.prod_target,
        )
    ...
    def test_addon_placement_wrong_addon_rejected(self) -> None:
        ...
        assert_rejected_with(
            self, self.agent, self.env.assemble(att),
            "placement must be signed by capacity-planner",
            target_identity=self.prod_target,
        )
    ...
    def test_addon_placement_target_not_in_decision_rejected(self) -> None:
        ...
        assert_rejected_with(
            self, self.agent, self.env.assemble(att),
            "action consistent with placement decision",
            target_identity=self.staging_target,
        )
    ...
    def test_remove_deployment_id_mismatch_rejected(self) -> None:
        ...
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(self.env.assemble(att), target_identity=self.staging_target)
    ...
    def test_namespace_constraint_fails_with_wrong_namespace(self) -> None:
        constraint = OutputConstraint(
            ...
            expression=(
                ...
                'm.content.metadata.namespace == "kube-system")'
            ),
        )
        si = self._addon_predicate(output_constraints=(constraint,))
        d = signed_put(self.env, "att-1", self.obs, si, SAMPLE_MANIFESTS)
        assert_rejected_with(
            self, self.agent, d, "must be in namespace kube-system",
            target_identity=self.prod_target,
        )

    def test_gvk_allowlist_plus_inline_strategy(self) -> None:
        from sigstore_tuf_bundle._test_helpers import allowed_gvks
    ...
    def test_user_constraint_fails_even_though_strategy_passes(self) -> None:
        ...
        d = unsigned_put(self.env, "att-1", si, SAMPLE_MANIFESTS)
        assert_rejected_with(
            self, self.agent, d, "no Deployments allowed",
            target_identity=self.prod_target,
        )
    ...
    def test_placement_batch_size_constraint(self) -> None:
        ...
        pb = sign_placement(
            self.placer, deployment_id="deploy-1", targets=("c1", "c2", "c3")
        )
        att = make_put_attestation(
            "att-1", si, SAMPLE_MANIFESTS,
            placement_targets=("c1", "c2", "c3"), placement_bundle=pb,
            placement_deployment_id="deploy-1",
        )
        assert_rejected_with(
            self, self.agent, self.env.assemble(att), "batch size limit",
    ...
    def test_swap_manifests_between_attestations(self) -> None:
        ...
        si = self._inline_predicate(
            manifests_a, 'target.labels.env == "prod"'
        )
        att = make_put_attestation("att-1", si, manifests_b)
        assert_rejected_with(
            self, self.agent, self.env.assemble(att),
            "manifests must match inline spec",
            target_identity=self.prod_target,
        )
    ...
    def test_expired_attestation_rejected(self) -> None:
        ...
        d = unsigned_put(self.env, "att-1", si, SAMPLE_MANIFESTS)
        assert_rejected_with(
            self, self.agent, d, "expired", target_identity=self.prod_target
        )
    ...
    def test_placement_evidence_cross_deployment_replay_rejected(self) -> None:
        ...
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(self.env.assemble(att), target_identity=self.prod_target)
        self.assertIn("deployment_id mismatch", str(ctx.exception))
    ...
    def test_forged_placement_evidence_wrong_key(self) -> None:
        ...
        evil_placer = self.env.addon_signer("not-registered")
        si = self._inline_addon(SAMPLE_MANIFESTS)
    ...
    def test_unknown_manifest_strategy_fails_closed(self) -> None:
        ...
        d = unsigned_put(self.env, "att-1", si, SAMPLE_MANIFESTS)
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(d, target_identity=self.prod_target)
    ...
    def test_forged_manifest_signature_untrusted_key(self) -> None:
        si = self._addon_predicate()
        forged = self.env.addon_signer("not-registered")
...
class FleetWideUpgradeTests(unittest.TestCase):
    ...
    def test_fleet_upgrade_explanation_shows_full_chain(self) -> None:
        ...
        explanation = self.agent.explain(d, target_identity=self.prod_target)
    ...
    def test_fleet_upgrade_wrong_target_rejected(self) -> None:
        ...
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(d, target_identity=self.staging_target)
    ...
    def test_fleet_upgrade_prior_constraints_carry_forward(self) -> None:
        ...
        assert_rejected_with(
            self, self.agent, d, "capi-provisioner",
            target_identity=self.prod_target,
        )
    ...
    def test_fleet_upgrade_untrusted_upgrade_signer_rejected(self) -> None:
        ...
        assert_rejected_with(
            self, self.agent, d, "unknown trust anchor",
            target_identity=self.prod_target,
        )
    ...
    def test_fleet_upgrade_cross_deployment_evidence_rejected(self) -> None:
        ...
        att = make_put_attestation(
            ...
            manifests,
            manifest_bundle=mb,
            placement_targets=("cluster-prod-1",),
            placement_bundle=stolen,
            placement_deployment_id="cluster-02",
        )
        d = self.env.assemble(
            att,
            prior_inputs={"cluster-01-v1": v1},
        ...
        assert_rejected_with(
            self, self.agent, d, "deployment_id mismatch",
            target_identity=self.prod_target,
        )

    def test_fleet_upgrade_replay_against_different_deployment_rejected(self) -> None:
        v1 = self._base(deployment_id="cluster-02")
        up = self._upgrade(target_deployments=("cluster-01",))
        d = signed_put(
            self.env, "cluster-02-v2", self.capi,
            DerivedInput("cluster-02", "deployment", "cluster-02-v1", "upgrade-1"),
            self._target_manifests(),
            prior_inputs={"cluster-02-v1": v1},
            update_attestations={"upgrade-1": up},
        )
        assert_rejected_with(
            self, self.agent, d, "placement",
            target_identity=self.prod_target,
        )
...
class GenerationFencingTests(unittest.TestCase):
    ...
    def test_generation_matches_target_state_put_accepted(self) -> None:
        ...
        self.assertTrue(self.agent.verify(d, target_identity=self.prod_target, current_fulfillment_state=FulfillmentState("cluster-01", 0)).valid)

    def test_stale_generation_put_rejected(self) -> None:
        d = signed_put(self.env, "cluster-01-v1", self.capi, self._base_input(expected_generation=1), self._target_manifests("1.29.5"))
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(d, target_identity=self.prod_target, current_fulfillment_state=FulfillmentState("cluster-01", 1))
    ...
    def test_generation_state_wrong_deployment_fails_closed(self) -> None:
        d = signed_put(self.env, "cluster-01-v1", self.capi, self._base_input(expected_generation=1), self._target_manifests("1.29.5"))
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(d, target_identity=self.prod_target, current_fulfillment_state=FulfillmentState("cluster-99", 0))
    ...
    def test_derived_generation_increments(self) -> None:
        ...
        self.assertTrue(self.agent.verify(d, target_identity=self.prod_target, current_fulfillment_state=FulfillmentState("cluster-01", 1)).valid)
    ...
    def test_derived_chain_generation_accumulates(self) -> None:
        ...
        self.assertTrue(self.agent.verify(d, target_identity=self.prod_target, current_fulfillment_state=FulfillmentState("cluster-01", 2)).valid)
    ...
    def test_derived_chain_stale_generation_rejected(self) -> None:
        ...
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(d, target_identity=self.prod_target, current_fulfillment_state=FulfillmentState("cluster-01", 3))
        self.assertIn("generation mismatch", str(ctx.exception))
    ...
    def test_reordered_updates_fail_when_preconditions_conflict(self) -> None:
        ...
        self.assertTrue(self.agent.verify(ok, target_identity=self.prod_target).valid)
        bad = signed_put(
        ...
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(bad, target_identity=self.prod_target)
        self.assertIn("precondition failed", str(ctx.exception))
    ...
    def test_preconditionless_updates_are_reorderable(self) -> None:
        ...
        for order in (("u1", "u2"), ("u2", "u1")):
            ...
            self.assertTrue(self.agent.verify(d, target_identity=self.prod_target).valid, order)
    ...
    def test_expected_generation_is_signed(self) -> None:
        ...
        assert_rejected_with(
            self, self.agent, d, "in-toto subject digest mismatch",
            target_identity=self.prod_target,
        )
