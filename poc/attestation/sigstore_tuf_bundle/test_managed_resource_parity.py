...
from sigstore_tuf_bundle.model import DerivedInput, RegisteredSelfTarget
from sigstore_tuf_bundle.sigstore import SigstoreBundle
...
class ManagedResourceEquivTests(unittest.TestCase):
    ...
    def test_happy_path_registered_self_target(self) -> None:
        ...
        self.assertTrue(self.agent.verify(d, target_identity={"id": "cluster-mgmt-addon"}).valid)
    ...
    def test_wrong_target_rejected(self) -> None:
        ...
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(d, target_identity={"id": "some-other-target"})
    ...
    def test_relation_not_found_in_bundle_rejected(self) -> None:
        ...
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(d, target_identity={"id": "cluster-mgmt-addon"})
    ...
    def test_relation_signature_invalid_rejected(self) -> None:
        ...
        import base64, copy
        document = rel.bundle.to_dict()
        document["dsseEnvelope"]["signatures"][0]["sig"] = base64.b64encode(
            b"\x00" * 64
        ).decode()
        bad = RegisteredSelfTarget(
            addon_id=rel.addon_id,
            resource_type="clusters",
            manifest_type=rel.manifest_type,
            bundle=SigstoreBundle(document),
        )
    ...
    def test_relation_signer_not_in_trust_anchor_rejected(self) -> None:
        ...
        with self.assertRaises(VerificationError) as ctx:
            self.agent.verify_or_raise(d, target_identity={"id": "rogue-addon"})
    ...
    def test_derived_managed_resource(self) -> None:
        ...
        # Hybrid has the platform planner authorize and produce the update.
        # The default POC policy is stricter, so relax the evidence-kind gate
        # only here to exercise that same derivation topology.
    ...
    def test_deployment_and_managed_resource_coexist(self) -> None:
        ...
        self.assertTrue(self.agent.verify(deploy_d, target_identity={"id": "target-1", "labels": {"env": "prod"}}).valid)
        ...
        self.assertTrue(self.agent.verify(mr_d, target_identity={"id": "cluster-mgmt-addon"}).valid)
