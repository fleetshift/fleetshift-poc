"""Offline delivery verification: embedded TUF + Sigstore Mode A + FleetShift policy."""
...
from .sigstore_sign import SigstoreBundle, verify_sigstore_bundle
from .tuf_store import TrustedMaterial, verify_embedded_tuf
...
def _verify_output_bundle(
    ...
    *,
    ...
    if bundle is None:
        return _ok(label, "unsigned"), None
    digest = sha256_bytes(canonical_json(payload))
    try:
        statement = verify_sigstore_bundle(
            bundle, trusted=trusted, expected_subject_digest=digest
        )
    ...
    if canonical_json(pred.get("payload")) != canonical_json(payload):
        return _fail(label, "signed payload mismatch"), None
    return _ok(label, f"signed by {bundle.identity.subject}"), bundle.identity.subject


def _relation_doc(resource_type: str) -> dict[str, str]:
    return {
...
def _derive_managed_resource_constraints(
    ...
    doc = _relation_doc(relation.resource_type)
    digest = sha256_bytes(canonical_json(doc))
    try:
        statement = verify_sigstore_bundle(
            relation.bundle, trusted=trusted, expected_subject_digest=digest
        )
    except Exception as exc:  # noqa: BLE001
        msg = str(exc).lower()
        if "digest" in msg or "subject" in msg:
            return (
                OutputConstraint(
    ...
    if relation.signer_id != content.addon_id:
        return (
            OutputConstraint(
                name=(
                    f"relation signer mismatch: "
                    f"signed by {relation.signer_id!r}, "
...
def _verify_signed_input(
    ...
    *,
    ...
    if now > signed.valid_until:
        return _fail(label, f"expired: {signed.bundle.identity.subject}"), None

    # Trust-anchor CEL constraints (tenant isolation, etc.).
    anchor = trusted.anchors[signed.bundle.trust_anchor_id]
    for cdoc in anchor.get("constraints", []):
        expr = cdoc["expression"]
        name = cdoc["name"]
        try:
            ...
            ok = evaluate_bool(
                ...
                {
                    "anchor": {
                        "anchor_id": signed.bundle.trust_anchor_id,
                        "attributes": anchor.get("attributes", {}),
                    },
                    "subject": {
                        "kind": "input",
                        "signer_id": signed.bundle.identity.subject,
            ...
            return _fail(
                label, f"trust anchor constraint evaluation failed: {exc}"
            ), None
        except Exception as exc:  # noqa: BLE001
            return _fail(label, f"trust anchor constraint failed: {name}: {exc}"), None
        if not ok:
            return _fail(label, f"trust anchor constraint failed: {name}"), None

    children.append(_ok(f"{attestation_id} sigstore", f"signed by {signed.bundle.identity.subject}"))
    return (
        _ok(label, f"verified against {signed.bundle.trust_anchor_id}", children),
        VerifiedInput(
    ...
    *,
    ...
    return (
        ...
        VerifiedInput(
            ...
            content_hash=sha256_bytes(canonical_json(derived_content.to_dict())),
...
def _verify_put(
    ...
    *,
    ...
    sig_result, manifest_signer = _verify_output_bundle(
    ...
    if output.placement_bundle is not None:
        ...
        pe_result, placement_signer = _verify_output_bundle(
    ...
    if output.manifest_bundle is not None:
        sig_ctx = {
            "signer_id": output.manifest_bundle.identity.subject,
            "trust_anchor_id": output.manifest_bundle.trust_anchor_id,
        }
    if output.placement_bundle is not None:
        placement_ctx: dict[str, Any] = {
            ...
            "signature": {
                "signer_id": output.placement_bundle.identity.subject,
                "trust_anchor_id": output.placement_bundle.trust_anchor_id,
            },
        }
    else:
        placement_ctx = {
...
def _check_generation(
    ...
    if verified_input.expected_generation is None:
        return _ok(label, "no expected_generation signed")
    if state is None:
        return _ok(
            label,
...
def _verify_attestation_core(
    ...
    *,
    ...
    gen_result = _check_generation(
        verified_input, current_fulfillment_state, att.attestation_id
    )
...
def verify_delivery(
    ...
    *,
    ...
    """Verify a preassembled delivery bundle entirely offline."""
    now = time.time() if now is None else now
    target_identity = target_identity or {}
    children: list[VerificationResult] = []

    try:
        trusted = verify_embedded_tuf(
            delivery.tuf_snapshot,
            bootstrap_root_keyids=delivery.bootstrap_root_keyids,
        )
    except Exception as exc:  # noqa: BLE001
        return _fail("tuf", f"embedded trust snapshot invalid: {exc}")
    children.append(_ok("tuf", "embedded trust snapshot verified"))
    ...
    # Nest attestation result under TUF when successful path includes both.
    if result.children:
        return VerificationResult(
            valid=result.valid,
            label=result.label,
            detail=result.detail,
            children=[children[0], *result.children]
            if children
            else result.children,
        )
    if not result.valid:
        return _fail(att.attestation_id, result.detail, [children[0], result])
    return _ok(att.attestation_id, result.detail, [children[0]])
...
def verify_attestation_or_raise(
    ...
    **kwargs: Any,
    ...
    result = verify_delivery(delivery, **kwargs)
    if not result.valid:
        # Include the full tree so callers can assert on nested failure details
        # (matches hybrid.verify_attestation / VerificationError behavior).
...
def explain_verification(
    ...
    **kwargs: Any,
    ...
    """Return the full verification tree (valid or not) without raising."""
    return verify_delivery(delivery, **kwargs)
