...
```
fleetshift-server/   Go gRPC server (management API, workflow engine, kind/k8s providers)
fleetshift-cli/      fleetctl CLI client
...
```bash
make build          # builds bin/fleetshift and bin/fleetctl
```
