...
// CEL int whether it's backed by a native integer column or JSONB
// text). The compiler derives it from the *other* side of a
// comparison/in -- e.g. `resource.generation > 4` derives
...
type ResolveContext struct {
	...
	// Fields backed by a JSONB shape that differs per resource_type
	// (e.g. resource.spec.*) require this to be non-nil.
	GuardedResourceType *domain.ResourceType

	// Bind registers v as a SQL bind parameter and returns its "$N"
	// placeholder. FieldResolver implementations must call this for
	// any filter-supplied *value* they need in the generated SQL --
	// e.g. a label key from resource.labels["team"] -- rather than
	// writing it into SQL text directly, so a key containing SQL
...
// structure, literals, in, parameter numbering, resource_type guard
// detection -- and knows about field paths only generically; a
// FieldResolver owns the actual row shape a path reads from (column
// names, JSONB extraction, schema-backed path validation). See the
// postgres package's query field resolver for this project's only
// implementation.
