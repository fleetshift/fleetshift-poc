...
func Run(t *testing.T, factory Factory) {
	...
	t.Run("Update", func(t *testing.T) {
		...
		snap := f.Snapshot()
		snap.State = domain.FulfillmentStateActive
		snap.ResolvedTargets = []domain.TargetID{"t1", "t2"}
		snap.UpdatedAt = laterTime
		snap.Generation = 3
		f = domain.FulfillmentFromSnapshot(snap)
	...
	t.Run("UpdateNotFound", func(t *testing.T) {
		repo := factory(t)
		err := repo.Update(context.Background(), domain.FulfillmentFromSnapshot(domain.FulfillmentSnapshot{ID: "nonexistent"}))
		if !errors.Is(err, domain.ErrNotFound) {
	...
	t.Run("Delete_CascadesStrategyRecords", func(t *testing.T) {
		...
		f2 := domain.FulfillmentFromSnapshot(domain.FulfillmentSnapshot{
			ID:        domain.FulfillmentID("f1"),
			State:     domain.FulfillmentStateCreating,
			CreatedAt: fixedTime,
			UpdatedAt: fixedTime,
	...
	t.Run("Update_ActiveWorkflowGen_SetAndClear", func(t *testing.T) {
		...
		snap := f.Snapshot()
		snap.ActiveWorkflowGen = &gen
		snap.UpdatedAt = fixedTime.Add(time.Minute)
		f = domain.FulfillmentFromSnapshot(snap)
		if err := repo.Update(ctx, f); err != nil {
		...
		snap = f.Snapshot()
		snap.ActiveWorkflowGen = nil
		snap.UpdatedAt = fixedTime.Add(2 * time.Minute)
		f = domain.FulfillmentFromSnapshot(snap)
		if err := repo.Update(ctx, f); err != nil {
	...
	t.Run("Update_AdvanceManifestStrategy_PersistsNewVersion", func(t *testing.T) {
		...
		got.AdvanceManifestStrategy(domain.ManifestStrategySpec{
			Type: domain.ManifestStrategyInline,
	...
	t.Run("Update_WithRolloutAndProvenance", func(t *testing.T) {
		...
		snap.UpdatedAt = fixedTime.Add(time.Minute)
		updated := domain.FulfillmentFromSnapshot(snap)
		updated.AdvanceRolloutStrategy(&domain.RolloutStrategySpec{
			Type:                  domain.RolloutStrategyImmediate,
			VersionConflictPolicy: domain.VersionConflictCompleteAll,
		}, fixedTime)
	...
	t.Run("Generation_UpdateAdvances", func(t *testing.T) {
		...
		snap := got.Snapshot()
		snap.Generation = got.Generation() + 1
		snap.UpdatedAt = fixedTime.Add(time.Minute)
		updated := domain.FulfillmentFromSnapshot(snap)
		if err := repo.Update(ctx, updated); err != nil {
			t.Fatalf("Update: %v", err)
