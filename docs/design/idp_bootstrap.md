...
Bootstrap config is **first-trust-only**: it installs an IdP when none exists yet, and does not re-apply or overwrite on later restarts. Name flags and env vars accordingly (`initial`, `bootstrap`, and the like) so operators do not treat them as a standing source of truth.
...
If the change cannot go through that API (wrong IdP, lockout, recovery), use the local administrative control channel — the same class of runtime-authority path as bootstrap — not another pass of startup config.
...
Co-located IdP details stay packaging concerns: loopback HTTPS, private CA material, UI origin contracts, peer readiness under the process supervisor. The server only sees an ordinary issuer URL and clients.

The sandbox entrypoint can always start Dex and feed that issuer into bootstrap config. Optionally accepting a different issuer and skipping Dex is a nice-to-have for that image, not a separate server mode.
...
Prefer a client per CLI and UI, as well as an additional OIDC client just for signature enrollment. Use a resource-server-specific audience check where the IdP supports them (Dex: cross-client trust / `audience:server:client_id:…`), otherwise we can relax that on the server side. For example, we could allow one of multiple audiences, matching the different clients. This is somewhat a misapplication of OAuth, but can be acceptable as we are trying to lower the barrier to entry).

For the signature enrollment flow, however, this is more sensitive, and should ideally really be its own client. In that case, though, its audience is intentionally more simplistic and well-define: in fact it **MUST** be the same client ID as the enrollment client.
...
| Item                                     | Notes                                                                                                                           |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Local-socket / CLI bootstrap             | Useful for production when IdP is not trivially known at start, and later for out-of-band recovery; not required to ship config-based bootstrap. Likely its own control API. |
| IdP update / replace after Day One       | Needed eventually; singular model, not sandbox-specific. Authenticated API first; local control channel when that is unavailable. |
...
- Exact config surface for bootstrap (issuer only vs full auth-method fields, default auth method id, audience set). I'm not sure whether this should also include trust signature policy, or if that should come from later UI set up. Ideally, we would define that up front too, or at least support defining it. Or, we just default it to the "easy mode" out of the box and let you override as an advanced use case.
