...
**Management-plane concurrency control.** The management plane exposes two orthogonal precondition mechanisms on mutation requests (e.g. resume). `etag` is a weak domain-state token (per AIP-154 and RFC 9110 Section 8.8.1, `W/`-prefixed) that changes whenever any API-visible state changes — including reconciliation state, resolved targets, and timestamps — not just generation-advancing mutations. `expected_generation` is an explicit version-lineage claim that the client signs into provenance; delivery agents verify it for anti-replay. Clients can combine both for exact-snapshot writes, supply only `expected_generation` for low-churn generation-bound writes, or omit both for unsigned legacy operations.
