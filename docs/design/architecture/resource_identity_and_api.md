...
Storage-wise, `resource_aliases` holds one row per `(namespace, key, value, source_extension_resource_uid)` rather than collapsing every contributor's claim into a single row keyed by `(namespace, key, value)` alone — this is what lets corroborating contributors coexist and lets the alias survive until its *last* contributor stops asserting it, with retraction scoped to `ON DELETE CASCADE` from `extension_resources` for the "resource disappears" case and an explicit delete for the "still exists but stopped reporting this key" case. Postgres enforces the two cross-contributor invariants above — value-claimed-by-a-different-resource and resource-has-a-different-value — with a pair of `EXCLUDE` constraints (via the `btree_gist` extension) rather than plain `UNIQUE` indexes, since a `UNIQUE` index can't express "these rows may coexist for the same contributor, but not otherwise." SQLite has no `EXCLUDE`-equivalent, so it enforces the same two invariants procedurally instead (see `extension_resource_repo.go`'s `resolveAliasesForWrite` in each backend).

The above describes `ReplaceInventory`'s Aliases field, which always carries a contributor's complete alias contribution. The incremental `ApplyInventoryDeltas` entry point can't use a single "complete contribution" field — most deltas are heartbeats that touch none of a contributor's aliases at all, and the rest usually touch one key, not the whole set — so it splits the same replace-per-contributor/additive-across-contributors contract into three narrower ops instead:

- **UpsertAliases** adds or updates specific `(namespace, key)` contributions, mirroring how `SetLabels` touches only the label keys it names.
- **DeleteAliases** retracts specific contributions by `(namespace, key)` alone — no value needed, since a contributor never holds two different values for the same key at once — mirroring `DeleteLabels`.
- **ReplaceAliases**, mutually exclusive with the other two in the same delta, offers the same "this is my complete contribution" convenience `ReplaceInventory`'s Aliases field gets, scoped to just the alias part of an otherwise-incremental call.

A delta using only UpsertAliases/DeleteAliases behaves like every other delta field: an alias absent from both is simply left unchanged, never retracted — that's deliberately different from `ReplaceInventory`, where omission always means "retract."

> Not yet implemented: only `ReplaceInventory`'s per-contributor replace (described above) is wired into `resource_aliases` today. `ApplyInventoryDeltas` still only folds in `UpsertAliases`; `DeleteAliases` and `ReplaceAliases` are validated (including the mutual-exclusivity and same-key-in-both-Upsert-and-Delete checks) but not yet acted on by either backend. `extensionresourcerepotest`'s `ApplyInventoryDeltas*Aliases*` tests pin the target contract down ahead of that landing.
