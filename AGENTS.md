...
- Prefer running tasks through Nx (`npx nx run <project>:<target>`, `npx nx run-many`, `npx nx affected`) for caching and dependency-aware execution. Taskfile commands still work directly (`task -l` for available tasks) — Nx wraps them.
