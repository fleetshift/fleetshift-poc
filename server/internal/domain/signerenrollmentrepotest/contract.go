// Package signerenrollmentrepotest provides a shared contract test suite for
// [domain.SignerEnrollmentRepository] implementations. The factory returns a
// [domain.Store] because signer enrollment tests operate across transactions
// (Begin/Commit/BeginReadOnly).
package signerenrollmentrepotest
...
// Factory creates a ready-to-use [domain.Store] backed by a clean database.
type Factory func(t *testing.T) domain.Store

// Run executes the full signer enrollment contract test suite against the
